	        	...
	        	switch (type)
	        	{
					...
					case 'reply':
						if (result.rsm.ajax_html)
						{
							...
							if (USER_ANSWERED)
							{
								$('.aw-replay-box.question').append('<p align="center">一个主题只能回复一次, 你可以在发言后 ' + ANSWER_EDIT_TIME + ' 分钟内编辑回复过的内容</p>');
							}
								...
								$.get(G_BASE_URL + '/search/ajax/search/?q=' + encodeURIComponent($(element).val()) + '&limit=5',function(result)
								{
									if (result.length > 0)
									{
										...
										$.each(result, function(i, e)
										{
											...
											{
												case 'questions' :
													ul.append('<li class="question"><a href="' + decodeURIComponent(result[i].url) + '">' + result[i].name + '&nbsp;<span class="color-999">' + result[i].detail.answer_count + ' 个回复</span></a></li>');
													break;
