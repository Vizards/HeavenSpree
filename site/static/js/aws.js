	        	...
	        	switch (type)
	        	{
					...
					case 'reply':
						...
						if (result.rsm.ajax_html)
						{
							...
							if (USER_ANSWERED)
							{
								$('.aw-replay-box').append('<p align="center">一个主题只能回复一次, 你可以在发言后 ' + ANSWER_EDIT_TIME + ' 分钟内编辑回复过的内容</p>');
							}
						}
	    ...
	    if (template)
	    {
	        ...
	        switch (type)
	        {
		        ...
		        case 'publish':
					...
					{
						$('.aw-publish-box .modal-title').html('向 ' + data.ask_user_name + ' 发布');
					}
...
AWS.User =
{
	...
	follow: function(selector, type, data_id)
	{
		if (selector.html())
	    {
	        ...
	        {
	        	selector.find('span').html(_t('已关注'));
	...
	question_thanks: function(selector, question_id)
	{
	    $.post(G_BASE_URL + '/question/ajax/question_thanks/', 'question_id=' + question_id, function (result)
	    {
	        ...
	        {
	            selector.html(selector.html().replace(_t('关注'), _t('已关注')));
