...
class main extends AWS_CONTROLLER
{
	...
	public function index_action()
	{
		...
		{
			H::redirect_msg(AWS_APP::lang()->_t('主题不存在或已被删除'), '/');
		}
		...
		if (is_digits($_GET['rf']) and $_GET['rf'])
		{
			...
			{
				$redirect_message[] = AWS_APP::lang()->_t('从主题 %s 跳转而来', '<a href="' . get_js_url('/question/' . $_GET['rf'] . '?rf=false') . '">' . $from_question['question_content'] . '</a>');
			}
		}
		...
		else if ($question_info['redirect'])
		{
			if ($target_question)
			{
				$message = AWS_APP::lang()->_t('此主题将跳转至') . ' <a href="' . get_js_url('/question/' . $question_info['redirect']['target_id'] . '?rf=' . $question_info['question_id']) . '">' . $target_question['question_content'] . '</a>';
		...
		$question_list = $this->model('posts')->get_posts_list('question', $_GET['page'], '8');

		TPL::assign('new_posts_list', $question_list);
	...
	public function index_square_action()
	{
		...
		{
			$this->crumb(AWS_APP::lang()->_t('热门的主题'));
		...
		else
		{
			...
			{
			$this->crumb(AWS_APP::lang()->_t('等待回复的主题'));
			}
			if ($_GET['sort_type'] == 'new')
				{
					$this->crumb(AWS_APP::lang()->_t('最新的主题'));
				}	
	...
	public function unverify_modify_action()
	{
		...
		{
			H::redirect_msg(AWS_APP::lang()->_t('主题不存在'), '/');
		}
