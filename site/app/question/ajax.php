...
class ajax extends AWS_CONTROLLER
{
	...
	public function save_invite_action()
	{
		...
		{
			H::ajax_json_output(AWS_APP::RSM(null, '-1', AWS_APP::lang()->_t('主题不存在或已被删除')));
		}
		...
		{
			H::ajax_json_output(AWS_APP::RSM(null, '-1', AWS_APP::lang()->_t('不能邀请自己回复主题')));
		}
		...
		{
			H::ajax_json_output(AWS_APP::RSM(null, '-1', AWS_APP::lang()->_t('该用户已经回复过该主题')));
		}

		if ($question_info['published_uid'] == $invite_user_info['uid'])
		{
			H::ajax_json_output(AWS_APP::RSM(null, '-1', AWS_APP::lang()->_t('不能邀请主题的发表者回复主题')));
		}
		...
		{
			$this->model('weixin')->send_text_message($weixin_user['openid'], "有会员在主题 [" . $question_info['question_content'] . "] 邀请了你进行回复", $this->model('openid_weixin_weixin')->redirect_url('/m/question/' . $question_info['question_id']));
		}
	...
	public function fetch_share_data_action()
	{
		switch ($_GET['type'])
		{
			case 'question':
				...
				$message = AWS_APP::lang()->_t('我看到一个不错的主题, 想和你分享:') . ' ' . $question_info['question_content'] . ' ' . $url;
			break;
			...
			case 'answer':
				...
				$message = AWS_APP::lang()->_t('我看到一个不错的主题, 想和你分享:') . ' ' . $question_info['question_content'] . ' - ' . $user_info['user_name'] . ": " . $answer_info['answer_content'] . ' ' . $url;
			break;
	...
	public function save_answer_comment_action()
	{
		...
		{
			H::ajax_json_output(AWS_APP::RSM(null, '-1', AWS_APP::lang()->_t('不能评论锁定的主题')));
		}
	...
	public function question_thanks_action()
	{
		...
		{
			H::ajax_json_output(AWS_APP::RSM(null, '-1', AWS_APP::lang()->_t('不能关注自己的主题')));
		}
	...
	public function question_answer_rate_action()
	{
		...
		{
			H::ajax_json_output(AWS_APP::RSM(null, - 1, AWS_APP::lang()->_t('已关注过该回复, 请不要重复关注')));
		}
	...
	public function save_answer_action()
	{
		...
		if ($question_info['lock'] AND ! ($this->user_info['permission']['is_administortar'] OR $this->user_info['permission']['is_moderator']))
		{
			H::ajax_json_output(AWS_APP::RSM(null, '-1', AWS_APP::lang()->_t('已经锁定的主题不能回复')));
		}
		...
		{
			H::ajax_json_output(AWS_APP::RSM(null, '-1', AWS_APP::lang()->_t('不能回复自己发布的主题，你可以修改主题内容')));
		}

		// 判断是否已回复过主题
		if ((get_setting('answer_unique') == 'Y') AND $this->model('answer')->has_answer_by_uid($question_info['question_id'], $this->user_id))
		{
			H::ajax_json_output(AWS_APP::RSM(null, '-1', AWS_APP::lang()->_t('一个主题只能回复一次，你可以编辑回复过的回复')));
		}
	...
	public function log_action()
	{
		...
		{
			H::ajax_json_output(AWS_APP::RSM(null, -1, AWS_APP::lang()->_t('指定主题不存在')));
		}
	...
	public function redirect_action()
	{
		...
		if ($question_info['lock'] AND ! ($this->user_info['permission']['is_administortar'] or $this->user_info['permission']['is_moderator']))
		{
			H::ajax_json_output(AWS_APP::RSM(null, '-1', AWS_APP::lang()->_t('锁定的主题不能设置重定向')));
		}
	...
	public function remove_question_action()
	{
		...
		{
			H::ajax_json_output(AWS_APP::RSM(null, '-1', AWS_APP::lang()->_t('对不起, 你没有删除主题的权限')));
		}
	...
	public function set_best_answer_action()
	{
		...
		{
			H::ajax_json_output(AWS_APP::RSM(null, - 1, AWS_APP::lang()->_t('主题已经存在最佳回复')));
		}
	...
	// @xnile
	// biaotai.me
	public function set_stick_action()
	{
		if (!$this->user_info['permission']['is_administortar'] AND !$this->user_info['permission']['is_moderator'])
		{
			H::ajax_json_output(AWS_APP::RSM(null, '-1', AWS_APP::lang()->_t('对不起, 你没有设置推荐的权限')));
		...
		{
			case 'set':
				$this->model('question')->set_stick($_POST['question_id']);
			break;

			case 'unset':
				$this->model('question')->unset_stick($_POST['question_id']);
			break;
		}
