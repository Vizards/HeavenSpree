...
class main extends AWS_CONTROLLER
{
	public function get_access_rule()
	{
		...
		if ($this->user_info['permission']['visit_question'] AND $this->user_info['permission']['visit_site'])
		{
			...
			$rule_action['actions'][] = 'hots';
			$rule_action['actions'][] =  'recommend';
		}
...
public function index_action()
	{
		...
		//最新文章
        $article_list = $this->model('article')->get_articles_list($category_info['id'], $_GET['page'], '8', 'add_time DESC');
...
public function hots_action(){
   if (is_mobile())
	{
		HTTP::redirect('/m/article/');
	}
	$this->crumb(AWS_APP::lang()->_t('热门文章'), '/article/');
	
	if ($_GET['feature_id'])
	{
		$article_list = $this->model('article')->get_articles_list_by_topic_ids($_GET['page'], get_setting('contents_per_page'), 'votes DESC', $this->model('feature')->get_topics_by_feature_id($_GET['feature_id']));
		$article_list_total = $this->model('article')->article_list_total;
		if ($feature_info = $this->model('feature')->get_feature_by_id($_GET['feature_id']))
		{
			$this->crumb($feature_info['title'], '/article/feature_id-' . $feature_info['id']);
	...
	{
		$article_list = $this->model('article')->get_articles_list($category_info['id'], $_GET['page'], get_setting('contents_per_page'), 'votes DESC');
	    $article_list_total = $this->model('article')->found_rows();
	}
