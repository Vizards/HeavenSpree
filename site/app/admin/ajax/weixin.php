...
/*
        ...
        {
            H::ajax_json_output(AWS_APP::RSM(null, -1, AWS_APP::lang()->_t('请添加要群发的文章或主题')));
        }
*/

        if (count($article_ids) + count($question_ids) > 9)
        {
            H::ajax_json_output(AWS_APP::RSM(null, -1, AWS_APP::lang()->_t('最多可添加 9 个文章和主题')));
        }
        ...
        if ($question_ids)
        {
            ...
            if (isset($error_msg))
            {
                H::ajax_json_output(AWS_APP::RSM(null, -1, AWS_APP::lang()->_t('上传主题作者的头像失败, 错误信息: %s', $error_msg)));
            }
        }
