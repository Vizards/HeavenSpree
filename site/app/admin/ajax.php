...
class ajax extends AWS_ADMIN_CONTROLLER
{
    ...
    public function remove_category_action()
    {
        ...
        {
            H::ajax_json_output(AWS_APP::RSM(null, -1, AWS_APP::lang()->_t('分类下存在内容, 请先批量移动主题到其它分类, 再删除当前分类')));
        }
    ...
    public function question_manage_action()
    {
        ...
        {
            H::ajax_json_output(AWS_APP::RSM(null, -1, AWS_APP::lang()->_t('请选择主题进行操作')));
        }
    ...
    public function statistic_action()
    {
        ...
        foreach ($statistic_tag AS $key => $val)
        {
            switch ($val)
            {
                ...
                case 'new_question':    // 新增主题
                case 'new_user':    // 新注册用户
                case 'user_valid':  // 新激活用户
                case 'new_topic':   // 新增话题
                case 'new_answer_vote': // 新增答案投票
                case 'new_answer_thanks': // 新增答案喜爱
                case 'new_favorite_item': // 新增收藏条目
                case 'new_question_thanks': // 新增主题喜爱
                case 'new_question_redirect': // 新增主题重定向
        ...
        switch ($_POST['action'])
        {
            case 'add_service_user':
                if ($service_info)
                {
                    ...
                    {
                        H::ajax_json_output(AWS_APP::RSM(null, -1, AWS_APP::lang()->_t('该用户已是回复用户')));
                    }
            ...
            case 'del_service_user':
                if ($service_info)
                {
                    if (!isset($service_info['last_msg_id']))
                    {
                        H::ajax_json_output(AWS_APP::RSM(null, -1, AWS_APP::lang()->_t('该用户不是回复用户')));
                    }
