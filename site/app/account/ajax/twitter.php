...
class ajax_twitter extends AWS_CONTROLLER
{
    ...
    public function register_action()
    {
        ...
        $this->model('openid_twitter')->bind_account(AWS_APP::session()->twitter_user, $uid);

        if (AWS_APP::session()->twitter_user['profile_image_url'])
        {
            $this->model('account')->associate_remote_avatar($uid, AWS_APP::session()->twitter_user['profile_image_url']);
        }
