	...
	public function save_topic_relation_action()
	{
		...
		switch ($_POST['type'])
		{
			case 'question':
				...
				{
					H::ajax_json_output(AWS_APP::RSM(null, '-1', AWS_APP::lang()->_t('锁定主题不能添加话题')));
				}
			break;
		...
		{
			H::ajax_json_output(AWS_APP::RSM(null, '-1', AWS_APP::lang()->_t('单个主题或文章话题数量最多为 %s 个, 请调整话题数量', get_setting('question_topics_limit'))));
		}
