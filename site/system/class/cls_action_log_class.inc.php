...
class ACTION_LOG
{
	...
	const ADD_QUESTION = 101;	// 添加主题

	const MOD_QUESTON_TITLE = 102;	// 修改主题标题

	const MOD_QUESTION_DESCRI = 103;	// 修改主题描述

	const ADD_REQUESTION_FOCUS = 105;	// 添加主题关注

	const REDIRECT_QUESTION = 107;	// 主题重定向

	const MOD_QUESTION_CATEGORY = 108;	// 修改主题分类

	const MOD_QUESTION_ATTACH = 109;	// 修改主题附件

	const DEL_REDIRECT_QUESTION = 110;	// 删除主题重定向

	const ANSWER_QUESTION = 201;	// 回复主题

	const ADD_AGREE = 204;	// 增加赞同

	const ADD_USEFUL = 206;	// 加喜爱作者

	const ADD_UNUSEFUL = 207;	// 主题没有帮助
	...
	public static function format_action_data($action, $uid = 0, $user_name = null, $associate_question_info = null, $associate_topic_info = null)
	{
		...
		switch ($action)
		{
			case self::ADD_QUESTION:
				if ($associate_topic_info)
				{
					$action_string = '<a href="' . $user_profile_url . '" ' . $user_link_attr . '>' . $user_name . '</a> 在 <a href="' . $topic_url . '" ' . $topic_link_attr . '>' . $associate_topic_info['topic_title'] . '</a> ' . AWS_APP::lang()->_t('话题发表了一个主题');
				}
				else if ($associate_question_info['anonymous'])
				{
					$action_string = AWS_APP::lang()->_t('匿名用户') . ' ' . AWS_APP::lang()->_t('发表了主题');
				}
				else
				{
					$action_string = '<a href="' . $user_profile_url . '" ' . $user_link_attr . '>' . $user_name . '</a> ' . AWS_APP::lang()->_t('发表了主题');
				}
				break;

			case self::ADD_REQUESTION_FOCUS:
				$action_string = '<a href="' . $user_profile_url . '" ' . $user_link_attr . '>' . $user_name . '</a> ' . AWS_APP::lang()->_t('关注了该主题');;
				break;
			...
			case self::ANSWER_QUESTION:
				...
				{
					$action_string = '<a href="' . $user_profile_url . '" ' . $user_link_attr . '>' . $user_name . '</a> ' . AWS_APP::lang()->_t('回复了主题');
				}
				break;
			...
			case self::ADD_TOPIC : // '添加话题',
				if ($associate_topic_info)
				{
					...
					{
						$action_string = '<a href="' . $user_profile_url . '" ' . $user_link_attr . '>' . $user_name . '</a> ' . AWS_APP::lang()->_t('将该主题添加到');
					...
					{
						$action_string = '<a href="' . $user_profile_url . '" ' . $user_link_attr . '>' . $user_name . '</a> ' . AWS_APP::lang()->_t('将该主题添加到') . ' <a href="' . $topic_url . '" ' . $topic_link_attr . '>' . $associate_topic_info['topic_title'] . '</a> ' . AWS_APP::lang()->_t('话题');
					}
				}
				else
				{
