...
class question_class extends AWS_MODEL
{
	 ...
	 *
	 * 增加主题内容
	 * @param string $question_content //主题内容
	 * @param string $question_detail  //主题说明
	 *
	...
	public function answer_question_invite($question_id, $recipients_uid)
	{
		...
		{
			$this->model('integral')->process($question_info['published_uid'], 'INVITE_ANSWER', get_setting('integral_system_config_invite_answer'), '邀请回复成功 #' . $question_id, $question_id);

			$this->model('integral')->process($recipients_uid, 'ANSWER_INVITE', -get_setting('integral_system_config_invite_answer'), '回复邀请回复 #' . $question_id, $question_id);
		}
		...
		if ($at_users = $this->model('question')->parse_at_user($message, false, true))
		{
			foreach ($at_users as $user_id)
			{
				...
				if ($weixin_user = $this->model('openid_weixin_weixin')->get_user_info_by_uid($user_id))
				{
					...
					{
						$this->model('weixin')->send_text_message($weixin_user['openid'], "有会员在主题 [" . $question_info['question_content'] . "] 评论中提到了您", $this->model('openid_weixin_weixin')->redirect_url('/m/question/' . $question_info['question_id']));
					}
				}
			}
		}
		...
		foreach ($log_list as $key => $log)
		{
			...
			switch ($log['associate_action'])
			{
				case ACTION_LOG::ADD_QUESTION :
					$title_list = $user_name_string . ' 添加了该主题</p><p>' . $log['associate_content'] . '</p><p>' . $log['associate_attached'] . '';
					break;

				case ACTION_LOG::MOD_QUESTON_TITLE : //修改主题标题

					$title_list = $user_name_string . ' 修改了主题标题';
				...
				case ACTION_LOG::MOD_QUESTION_DESCRI : //修改主题

					$title_list = $user_name_string . ' 修改了主题内容';
				...
				case ACTION_LOG::ADD_TOPIC : //添加话题
					...
					$title_list = $user_name_string . ' 给该主题添加了一个话题 <p><a href="topic/' . $topic_info['url_token'] . '">' . $log['associate_content'] . '</a>';
				...
				case ACTION_LOG::DELETE_TOPIC : //移除话题
					...
					$topic_info = $this->model('topic')->get_topic_by_id($log['associate_attached']);
					$title_list = $user_name_string . ' 移除了该主题的一个话题 <p><a href="topic/' . $topic_info['url_token'] . '">' . $log['associate_content'] . '</a>';
				...
				case ACTION_LOG::MOD_QUESTION_CATEGORY : //修改分类
					...
					$title_list = $user_name_string . ' 修改了该主题的分类 <p><a href="explore/category-' . $log['associate_attached'] . '">' . $log['associate_content'] . '</a>';
				...
				case ACTION_LOG::MOD_QUESTION_ATTACH : //修改附件
					...
					$title_list = $user_name_string . ' 修改了该主题的附件 ';

					break;

				case ACTION_LOG::REDIRECT_QUESTION : //主题重定向

					$question_info = $this->get_question_info_by_id($log['associate_attached']);

					if ($question_info)
					{
						$title_list = $user_name_string . ' 将主题重定向至：<a href="question/' . $log['associate_attached'] . '">' . $question_info['question_content'] . '</a>';
					}

					break;

				case ACTION_LOG::DEL_REDIRECT_QUESTION : //取消主题重定向


					$title_list = $user_name_string . ' 取消了主题重定向 ';
	...
	public function question_thanks($question_id, $uid, $user_name)
	{
		...
		else
		{
			...
			$this->model('integral')->process($uid, 'QUESTION_THANKS', get_setting('integral_system_config_thanks'), '关注主题 #' . $question_id, $question_id);

			$this->model('integral')->process($question_info['published_uid'], 'THANKS_QUESTION', -get_setting('integral_system_config_thanks'), '主题被关注 #' . $question_id, $question_id);
...
/* 置顶
	   ...
	   biaotai.me
	*/
	public function set_stick($question_id)
	{
		$this->update('question', array(
			'is_stick' => 1
		), 'question_id = ' . intval($question_id));
	...
	/* 置顶
	   ...
	   biaotai.me
	*/
	public function unset_stick($question_id)
	{
		$this->update('question', array(
			'is_stick' => 0
		), 'question_id = ' . intval($question_id));
