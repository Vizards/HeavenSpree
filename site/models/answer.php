...
class answer_class extends AWS_MODEL
{
	...
	public function remove_answer_by_id($answer_id)
	{
		if ($answer_info = $this->model('answer')->get_answer_by_id($answer_id))
		{
			...
			$this->delete('answer_thanks', 'answer_id = ' . intval($answer_id));	// 删除喜爱
			...
			foreach ($at_users as $user_id)
			{
				if ($user_id != $uid)
				{
					...
					if ($weixin_user = $this->model('openid_weixin_weixin')->get_user_info_by_uid($user_id))
					{
						...
						$this->model('weixin')->send_text_message($weixin_user['openid'], $answer_user['user_name'] . " 在主题 [" . $question_info['question_content'] . "] 的答案评论中提到了您", $this->model('openid_weixin_weixin')->redirect_url('/m/question/' . $question_info['question_id'] . '?answer_id=' . $answer_info['answer_id'] . '&single=TRUE'));
					}
				}
			}
		}
	...
	public function user_rate($type, $answer_id, $uid, $user_name)
	{
		...
		if ($type == 'thanks')
		{
			$this->model('integral')->process($uid, 'ANSWER_THANKS', get_setting('integral_system_config_thanks'), '喜爱回复 #' . $answer_info['answer_id'], $answer_info['answer_id']);

			$this->model('integral')->process($answer_info['uid'], 'THANKS_ANSWER', -get_setting('integral_system_config_thanks'), '回复被喜爱 #' . $answer_info['answer_id'], $answer_info['answer_id']);
		}
	...
	public function set_best_answer($answer_id)
	{
		...
		$this->model('integral')->process($answer_info['uid'], 'BEST_ANSWER', get_setting('integral_system_config_best_answer'), '主题 #' . $answer_info['question_id'] . ' 最佳回复');
