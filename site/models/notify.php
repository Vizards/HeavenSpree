...
class notify_class extends AWS_MODEL
{
	...
	const TYPE_NEW_ANSWER	= 102;	// 关注的主题增加了新回复
	const TYPE_COMMENT_AT_ME	= 103;	// 有评论@提到我
	const TYPE_INVITE_QUESTION	= 104;	// 被人邀请主题主题
	const TYPE_ANSWER_COMMENT	= 105;	// 我的回复被评论
	const TYPE_QUESTION_COMMENT	= 106;	// 我的主题被评论
	const TYPE_ANSWER_AGREE	= 107;	// 我的回复收到赞同
	const TYPE_ANSWER_THANK	= 108;	// 我的回复收到喜爱
	const TYPE_MOD_QUESTION	= 110;	// 我发布的主题被编辑
	const TYPE_REMOVE_ANSWER	= 111;	// 我发表的回复被删除

	const TYPE_REDIRECT_QUESTION	= 113;	// 我发布的主题被重定向
	const TYPE_QUESTION_THANK	= 114;	// 我发布的主题收到喜爱
	const TYPE_CONTEXT	= 100;	// 纯文本通知

	const TYPE_ANSWER_AT_ME	= 115;	// 有回复 @ 提到我
	const TYPE_ANSWER_COMMENT_AT_ME	= 116;	// 有回复评论 @ 提到我
	...
	const TYPE_QUESTION_APPROVED = 133; // 主题通过审核
	const TYPE_QUESTION_REFUSED = 134; // 主题未通过审核
	...
	public function format_notification($data)
	{
		foreach ($data AS $key => $val)
		{
			if ($val['extend_count'] > 1)
			{
				...
				{
					case self::CATEGORY_QUESTION:
						$data[$key]['message'] = $val['extend_count'] . ' ' . AWS_APP::lang()->_t('项关于主题') . ' <a href="' . $val['key_url'] . '">' . $val['title'] . '</a>';
				...
				foreach($val['extend_details'] AS $action_type => $extend)
				{
					...
					switch ($action_type)
					{
						case self::TYPE_ANSWER_AT_ME:
							$data[$key]['extend_message'][] = AWS_APP::lang()->_t('他们在回复中提到了你') . ': ' . $users_list;
							break;

						case self::TYPE_COMMENT_AT_ME:
							$data[$key]['extend_message'][] = AWS_APP::lang()->_t('他们在主题中的评论提到了你') . ': ' . $users_list;

							break;

						case self::TYPE_ANSWER_COMMENT_AT_ME:
							$data[$key]['extend_message'][] = AWS_APP::lang()->_t('他们在回复中的评论提到了你') . ': ' . $users_list;
						...
						case self::TYPE_NEW_ANSWER:
							$data[$key]['extend_message'][] = AWS_APP::lang()->_t('%s 个新回复, 按回复人查看', $extend['count']) . ': ' . $users_list;
						...
						case self::TYPE_ANSWER_THANK:
							$data[$key]['extend_message'][] = AWS_APP::lang()->_t('%s 个新喜爱, 按喜爱者查看', $extend['count']) . ': ' . $users_list;

							break;

						case self::TYPE_MOD_QUESTION:
							$data[$key]['extend_message'][] = AWS_APP::lang()->_t('%s 次编辑主题, 按编辑者查看', $extend['count']) . ': ' . $users_list;

							break;

						case self::TYPE_REDIRECT_QUESTION:
							$data[$key]['extend_message'][] = $users_list . ' ' . AWS_APP::lang()->_t('重定向了你发布的主题');
						...
						case self::TYPE_INVITE_QUESTION:
							$data[$key]['extend_message'][] = $users_list . ' ' . AWS_APP::lang()->_t('邀请你参与主题');
			...
			else
			{
				switch ($val['action_type'])
				{
					...
					case self::TYPE_NEW_ANSWER:
						...
						$data[$key]['message'] .= ' ' . AWS_APP::lang()->_t('回复了主题') . ' <a href="' . $val['key_url'] . '">' . $val['title'] . '</a>';
					...
					case self::TYPE_COMMENT_AT_ME:
						$data[$key]['message'] = '<a href="' . $val['p_url'] . '">' . $val['p_user_name'] . '</a> ' . AWS_APP::lang()->_t('在主题') . ' <a href="' . $val['key_url'] . '">' . $val['title'] . '</a> ' . AWS_APP::lang()->_t('中的评论提到了你');
					...
					case self::TYPE_ANSWER_AT_ME:
						...
						$data[$key]['message'] .= ' ' . AWS_APP::lang()->_t('在主题') . ' <a href="' . $val['key_url'] . '">' . $val['title'] . '</a> ' . AWS_APP::lang()->_t('中的回复提到了你');
					...
					case self::TYPE_ANSWER_COMMENT_AT_ME:
						...
						$data[$key]['message'] .= ' ' . AWS_APP::lang()->_t('在主题') . ' <a href="' . $val['key_url'] . '">' . $val['title'] . '</a> ' . AWS_APP::lang()->_t('回复评论中提到了你');
					break;

					case self::TYPE_INVITE_QUESTION:
						$data[$key]['message'] = '<a href="' . $val['p_url'] . '">' . $val['p_user_name'] . '</a> ' . AWS_APP::lang()->_t('邀请你参与主题') . ' <a href="' . $val['key_url'] . '">' . $val['title'] . '</a>';

						break;

					case self::TYPE_ANSWER_COMMENT:
						$data[$key]['message'] = '<a href="' . $val['p_url'] . '">' . $val['p_user_name'] . '</a> ' . AWS_APP::lang()->_t('评论了你在主题') . ' <a href="' . $val['key_url'] . '">' . $val['title'] . '</a> ' . AWS_APP::lang()->_t('中的回复');

						break;

					case self::TYPE_QUESTION_COMMENT:
						$data[$key]['message'] = '<a href="' . $val['p_url'] . '">' . $val['p_user_name'] . '</a> ' . AWS_APP::lang()->_t('评论了你发表的主题') . ' <a href="' . $val['key_url'] . '">' . $val['title'] . '</a>';

						break;

					case self::TYPE_ANSWER_AGREE:
						$data[$key]['message'] = '<a href="' . $val['p_url'] . '">' . $val['p_user_name'] . '</a> ' . AWS_APP::lang()->_t('赞同了你在主题') . ' <a href="' . $val['key_url'] . '">' . $val['title'] . '</a> ' . AWS_APP::lang()->_t('中的回复');

						break;

					case self::TYPE_ANSWER_THANK:
						$data[$key]['message'] = '<a href="' . $val['p_url'] . '">' . $val['p_user_name'] . '</a> ' . AWS_APP::lang()->_t('喜爱了你在主题') . ' <a href="' . $val['key_url'] . '">' . $val['title'] . '</a> ' . AWS_APP::lang()->_t('中的回复');

						break;

					case self::TYPE_MOD_QUESTION:
						$data[$key]['message'] = '<a href="' . $val['p_url'] . '">' . $val['p_user_name'] . '</a> ' . AWS_APP::lang()->_t('编辑了你发布的主题') . ' <a href="' . $val['key_url'] . '">' . $val['title'] . '</a>';

						break;

					case self::TYPE_REMOVE_ANSWER:
						$data[$key]['message'] = '<a href="' . $val['p_url'] . '">' . $val['p_user_name'] . '</a> ' . AWS_APP::lang()->_t('删除了你在主题') . ' <a href="' . $val['key_url'] . '">' . $val['title'] . '</a> ' . AWS_APP::lang()->_t('中的回复');

						break;

					case self::TYPE_REDIRECT_QUESTION:
						$data[$key]['message'] = '<a href="' . $val['p_url'] . '">' . $val['p_user_name'] . '</a> ' . AWS_APP::lang()->_t('重定向了你发表的主题') . ' <a href="' . $val['key_url'] . '">' . $val['title'] . '</a>';

						break;

					case self::TYPE_QUESTION_THANK:
						$data[$key]['message'] = '<a href="' . $val['p_url'] . '">' . $val['p_user_name'] . '</a> ' . AWS_APP::lang()->_t('喜爱了你发表的主题') . ' <a href="' . $val['key_url'] . '">' . $val['title'] . '</a>';
					...
					case self::TYPE_ARTICLE_APPROVED:
						$data[$key]['message'] = AWS_APP::lang()->_t('你发表的文章 %s 审核通过', '<a href="' . $val['key_url'] . '">' . $val['title'] . '</a>');

						break;

					case self::TYPE_ARTICLE_REFUSED:
						$data[$key]['message'] = AWS_APP::lang()->_t('你发表的文章 %s 审核未通过', $val['title']);

						break;

					case self::TYPE_QUESTION_APPROVED:
						$data[$key]['message'] = AWS_APP::lang()->_t('你发表的主题 %s 审核通过', '<a href="' . $val['key_url'] . '">' . $val['title'] . '</a>');

						break;

					case self::TYPE_QUESTION_REFUSED:
						$data[$key]['message'] = AWS_APP::lang()->_t('你发表的主题 %s 审核未通过', $val['title']);
