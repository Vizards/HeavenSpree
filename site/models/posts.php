...
class posts_class extends AWS_MODEL
{
	...
	*/
	public function get_posts_list($post_type, $page = 1, $per_page = 10, $sort = null, $topic_ids = null, $category_id = null, $answer_count = null, $day = 30, $is_recommend = false)
	{
		...
		// @xnile 只有第一页显示置顶贴，兼顾手机点更多加载的时候不会每次都把置顶的帖子加载出来
		// 如果想实现pc版每页都带有置顶贴按时这个方法稍修改下即可
		if ($page < 2){
			$where_[] = "is_stick = 1";
			$posts_index_top = $this->fetch_page('posts_index', implode(' AND ', $where_), $order_key, $page = 1, $per_page);
			
			//return $this->process_explore_list_data($posts_index);
			return $this->process_explore_list_data(array_merge($posts_index_top,$posts_index));
			// var_dump($this->process_explore_list_data(array_merge($posts_index_top,$posts_index)));
		} else {
			return $this->process_explore_list_data($posts_index);
		}
	...
	public function process_explore_list_data($posts_index)
	{
		...
		if ($project_ids)
		{
			...
			$project_attachs = $this->model('publish')->get_attachs('project',$project_ids,'min');//这里新增
		}
